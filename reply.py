...
parser = argparse.ArgumentParser(
    description="Reply service. Leave running in the background."
)
parser.add_argument(
    "-c",
    "--cfg",
        ...
        if notification["type"] == "mention":  # if we're mentioned:
            ...
            for post in context["ancestors"]:
                if post["account"]["id"] == me:
                    pin = post[
                        "id"
                    ]  # Only used if pin is called, but easier to call here
                    posts += 1
            ...
            if (mention == "pin") or (mention == "unpin"):  # check for keywords
                ...
                client.status_post(
                    ...
                    post_id,
                    visibility=visibility,
                    spoiler_text=cfg["cw"] if cfg["cw_reply"] else None,
                )  # send toost
